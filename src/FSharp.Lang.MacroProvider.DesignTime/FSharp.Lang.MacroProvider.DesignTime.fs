module FSharp.Lang.MacroProviderImplementation
...
type BasicErasingProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("FSharp.Lang.MacroProvider.DesignTime", "FSharp.Lang.MacroProvider.Runtime")], addDefaultProbingLocation=true)
...
type BasicGenerativeProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("FSharp.Lang.MacroProvider.DesignTime", "FSharp.Lang.MacroProvider.Runtime")])

    let ns = "FSharp.Lang.MacroProvider"
    let asm = Assembly.GetExecutingAssembly()
